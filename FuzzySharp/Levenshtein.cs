...
namespace FuzzySharp
{
    public static class Levenshtein
    {
        ...
        // Special Case
        public static double GetRatio(string input1, string input2)
        {
            int len1 = input1.Length;
            int len2 = input2.Length;
            int lensum = len1 + len2;

            int editDistance = EditDistance(input1, input2);

            return editDistance == 0 ? 1 : (lensum - editDistance) / (double)lensum;
        }

        private static int EditDistance(string value1, string value2)
        {
            if (value2.Length == 0)
            {
