...
namespace FuzzySharp.SimilarityRatio.Strategy.Generic
{
    internal class PartialRatioStrategy<T> where T : IEquatable<T>
    {
        public static int Calculate(ReadOnlySpan<T> input1, ReadOnlySpan<T> input2)
        {
            ...
            MatchingBlock[] matchingBlocks = Levenshtein.GetMatchingBlocks(shorter.ToArray(), longer.ToArray());
            ...
            foreach (var matchingBlock in matchingBlocks)
            {
                ...
                double ratio = Levenshtein.GetRatio(shorter.ToArray(), longSubstr.ToArray());
